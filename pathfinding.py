...
def heuristic(a, b, type=0, weight=1.0):
    if type == 0:  # Manhattan
        ...
        return weight * math.sqrt(
            (a.coordX - b.coordX) ** 2 + (a.coordY - b.coordY) ** 2
        )


def a_star_search(
    grid, start, goal, heuristicType=0, roadBias=True, straightBias=False, hWeight=1.0
):
    # Initialized
    open_set = []
    ...
    while open_set:
        ...
        for direction in DIRECTIONS:
            ...
            if 0 <= neighbor_x < len(grid) and 0 <= neighbor_y < len(grid):
                ...
                if roadBias:
                    tentative_g_score = (
                        g_score[current] + terrain_costs[neighbor.terrain_type]
                    )
                else:
